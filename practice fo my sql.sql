...
select * from test2 where salary > 25000;
select * from test2 where not salary > 25000;
select * from test2 where salary != 25000;
...
## How to add new constraint Index in a table or multiple file ko ek sath unique kaise banaye
...
select * from test11 as t11 left join test12 as t12 on t11.id = t12.id order by email_id;

#___________________________________________________________________________________________#

## How to use Stored Procedures - 
## jab hum table create kartey hai to ye automatically create ho jata hai.

/* niche diya gaya code humko Stored Procedures m likhna hai
or jo create procedure ke agey name likha hai ye isi naam se call hoga

CREATE PROCEDURE `new_procedure` (in marks float)
BEGIN
select * from test12 where test12.marks > marks;
END

or phir apply karna hai
*/
...
(1, 100, 'manish', 'data science', 50),
(2, 101, 'mohan', 'machine learning', 55),
(3, 102, 'dharma', 'math', 53),
(4, 103, 'tillu', 'history', 52),
(5, 104, 'killu', 'geography', 51),
(6, 105, 'pillu', 'drawing', 59);

select * from test12 where student_code = 102;
select firstname from test12;
/*
CREATE DEFINER=`root`@`localhost` PROCEDURE `stu_rank`(in marks float)
BEGIN
select * from test12 where test12.marks > marks;
select count(id) as stu_count from test12;
END
*/
call stu_rank(52);

/*
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_details`(in marks float)
BEGIN
select * from test12 where test12.marks > marks;
select count(id) as total from test12;
END
*/
call new_details(55);

/*
CREATE DEFINER=`root`@`localhost` PROCEDURE `student_limit`(in stu_limit int)
BEGIN
select * from test12 limit stu_limit;
select count(id) as stu_colum from test12;
ENDstu_rank
*/

call student_limit(3);

/*
CREATE DEFINER=`root`@`localhost` PROCEDURE `maximum_marks`(OUT highest float)
BEGIN
select max(marks) into highest from test12;
END
*/

call maximum_marks(@output);
select @output;

call display_marks(30);

/*
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_details`(inout input varchar(30))
BEGIN
select marks into input from test12 where student_code = input;
END
*/

set  @bello= 101;
call fetch_details(@bello);
select @bello;

#___________________________________________________________________________________________#

## How to create Index - ye execution ki speed badha deta hai
...
on test13(subject_name);

select * from test13 where subject_name = 'math';

alter table test13 drop index idx_subject;

#___________________________________________________________________________________________#

## How to make Views file - iska use hum jab kartey hai jaise humko
## sales team ko specify details send karni ho jaise name phone or extra.
...
select student_code, firstname from test14;
